...
std::shared_ptr<void> ServiceFactory::create(ServiceType serviceType,
		std::shared_ptr<DataManager> dataMng, CmdArgs const& args, int threads_per_device)
{
#ifndef CUDA
	(void) args;
#endif
	switch(serviceType) {
		...
		case ServiceType::GPUEnergyService6DModes:
			return std::shared_ptr<void>( new GPUEnergyService6DModes<REAL>(dataMng, args.deviceIds, threads_per_device));
#endif
		default:
